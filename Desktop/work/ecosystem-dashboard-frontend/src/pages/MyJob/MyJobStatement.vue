...
<template>
  <div class="bg-white pa-5 rounded-lg">
    ...
    <v-progress-linear
        v-if="isRequestLoading('getApplicationTypesApi')"
        indeterminate
        color="#2988F5"
    />
    ...
    <div
        v-else
        class="statement__filters"
    >
      <v-btn
          v-for="(item, index) in applicationTypes"
          variant="outlined"
          :key="index"
          :class="['statement__filters__btn no-uppercase rounded-lg', {
            'statement__filters__btn--active': +$route.query.type === +item.id
          }]"
          @click="changeAppType(item.id)"
      >
        {{ item.name }}
      </v-btn>
    </div>
